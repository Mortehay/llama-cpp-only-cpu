...
Corrected after first writing, and the correction matters more than most of
what it replaced: this ADR claimed that 103 cells recovered from 12 keyed
...
above `measure.MIN_TRAIN_SIDE`. They are written to `images/recovered/`:

    make key-checkerboard      # 12 files -> images/recovered/keyed
    make recover-cells         # -> images/recovered/cells, 103 cells

**They are not pixel art, and an earlier version of this ADR said they were.**
That claim - "the only training-grade character material here" - was made from
thumbnails and never checked against the gate this same document defines. Run
the gate over them and all 103 are rejected, every one on the same criterion:
many colours, no pixel grid.

The measurement that settles it: real pixel art is flat NxN blocks, so
collapsing each block to its mean and comparing is lossless.

| set | block round-trip error |
|---|---|
| hand-made 64x96 references (control) | **0.00** (p10 and p90 also 0.00) |
| 1024x1024 sprite references | 7.73 |
| the 103 recovered cells | 10.79, at every factor 2..12 |

A 32x32 patch of one cell holds 985 distinct colours out of 1024 pixels. These
are an imitation of pixel art: blocky at thumbnail size, per-pixel noise
underneath.

Where they came from is NOT established, and it is worth being careful about
because the tempting answer is unfalsifiable from here. What is measured: the
sources are 1024x1024 PNGs, and their transparency is painted into the pixels
as a checker pattern rather than carried in an alpha channel. What that is
consistent with: art generated by a model that had seen checkerboards in its
own training data; equally, a screenshot of an editor window; equally, real
pixel art that went through JPEG somewhere and was saved back to PNG. All three
produce per-pixel noise, and the round-trip measurement cannot separate them.

The training consequence is the same in all three cases and does not depend on
choosing between them: **the noise is what gets learned.** That is the finding.
The provenance is a guess, and this ADR has already been wrong once this week by
letting a plausible story stand in for a measurement.

What the recovery is still good for: the alpha is genuine, the cutouts are
clean, and the framing is right - one subject, centred, fully inside the crop.
As silhouette or composition material they hold up. As a style reference for
pixel art they do not, and training on them would teach the noise.
...
**The NEAREST rule routed all 103 cells to LANCZOS, and that was correct.**
This is recorded as a defect found and then unfound, because the unfinding is
the more useful half.

The test for "is this pixel art" was a small palette or a clean pixel grid, and
the recovered cells have neither: 4k-27k distinct colours, drawn at 1:1. Read
as "the style shades heavily and its edges still STEP", a third signal was
added - `edge_softness`, measuring what fraction of colour changes inside the
subject are ramps rather than steps. It separated the sets tidily (cells p90
0.45, painted art p10 0.52) and sent 95 of 103 cells back to NEAREST.

The premise was never tested. The cells have no grid at all (block round-trip
error 10.79 against a control's 0.00), so NEAREST protects nothing there and
merely magnifies per-pixel noise across a 4-6x upscale to 1024.

And the signal was inverted. Across all 483 references it fired ALONE - where
grid and palette both said no - on 65 files, and every one of those has block
error >= 1.24, median 9.42. It never once fired on art with a real grid,
because signals 1 and 2 had already claimed those. Its entire effect was to
route grid-less art to NEAREST.

Removed. `grid_round_trip_error` replaces it, and decides nothing: it is logged
per run, so a dataset's nature is recorded while it is being trained on instead
of being inferred later from a disappointing adapter. A run where no image has
a real grid says so in the log, and still runs - that is the user's call.

The lesson is the same one this ADR's testing section reaches from the other
direction: a signal built to fit a belief, then tested against that belief,
tests nothing. `edge_softness` had a passing test. The test was real; the
belief underneath it was not.
...
**Nothing in this repo satisfies all eight.** The 103 recovered cells satisfy
1-4 and 6-8 - one subject, centred, clean alpha, adequate size, no duplicates -
...
|---|---|---|---|
| ref_sprite_0c55cfbd4fa7 | 64x96 | 4 | 4 |
| ref_sprite_6a267730aeeb | 64x96 | 4 | 4 |
| ref_sprite_944dd81d28ab | 64x96 | 4 | 4 |
| ref_sprite_b9261a77ed6b | 64x96 | 4 | 4 |
| ref_sprite_d76876f7b9a4 | 64x96 | 4 | 4 |
| ref_sprite_1d82e1d009be | 64x96 | 4 | 5 |
| ref_sprite_344d56282766 | 64x96 | 4 | 5 |
| ref_sprite_7bb1b55f8708 | 64x96 | 4 | 5 |
| ref_core_08e39eb3c931 | 408x396 | 3 | 9 |
| ref_core_22e10326d210 | 312x273 | 3 | 11 |
| ref_core_a0f76603de29 | 1250x1250 | 10 | 16 |
...
An earlier draft of this section said there were eight, all `sprite`. **Six of
them are in `core`** - the tab this ADR spends most of its length criticising
as painted concept art. They are not concept art; they are palette-locked pixel
art at a 3-10x grid, filed under the wrong kind. That has a consequence for D2:
splitting the adapters by `kind` splits them by a label that is wrong for these
six, and they are a sixth of all the pixel art here. Recategorising them is a
data change, so it is the user's call and not made here - but the eight 64x96
sprites plus these six is 14, which is the first number in this document that
clears `MIN_IMAGES`.

All six `core` files were opened and looked at, not inferred from the numbers -
a drowned spirit, a bat, a serpent, a treant, a hound, a hooded figure. Every
one a single creature, palette-locked, on a flat backdrop. That check is not
optional here: the last three files this audit called `keep` on the sprite side
turned out to be a tree, a plinth and an archway, and the measurements said
nothing about it either time.

**Two of them need cropping before training.** `ref_core_a0f76603de29`
(1250x1250) and `ref_core_68076e9e3f53` (1280x1280) are mostly empty margin -
the subject occupies roughly a third of the frame. Under the new `--fit pad`
default that margin is preserved faithfully all the way to 1024, so most of the
training resolution would be spent on white. Crop to the subject's bounding box
first. This is a real cost of padding and the honest counterweight to D3's
argument for it: padding never silently discards a subject, and it never
rescues a badly framed one either.

They are the reference for what to look for. The first place to look for more
is whoever made them.

Rule 9, added because it nearly happened: **check that the art is pixel art
before believing it is.** `grid_round_trip_error` is one line, and it would have
saved this document two wrong claims. Blockiness at thumbnail size is not
evidence - a generated image imitating pixel art passes that test by eye every
time, which is what it was optimised to do.

One thing to keep out: **do not train on this pipeline's own output**, or on
anyone else's. Migration 013 records the reason - it is how a style collapses.
Worth reading alongside the recovered cells above: painted-on transparency and
per-pixel noise are what model output looks like, and those 12 sheets entered
this repo as references without anyone asking where they came from.
